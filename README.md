...
**Improving Unitary Parameterization**: We realized that the Gel Mann matrices based parameterization of the unitaries in the model used significantly more trainable parameters than the theoretical lower bound (total gm parameters = $2^{2n}-1$ v.s theoretical lower bound = $2^{n + 1}-2$). We found a paper[2] which presents an ansatz for quantum state preparation using uniformally controlled rotations. We used this as the basis of an alternate parameterization scheme with $2^{n + 2}-5$ total parameters. We then showed that this new scheme was able to achieve the same performance standards as the original model, now with fewer total parameters (thus faster training). In our particular case we realized a 8x reduction in total trainable parameters per 3 qubit convolutional layer
...
In order to translate this concept to the quantum equivalent we have to consider parameterized quantum circuits. In this case, our *tunable knobs* are the parameters we use to generate unitary operators which will act on the qubits in our circuit. This paper[1] uses the following parameterization: $\hat{U}(\vec{\theta})=e^{\sum_{i}^{N}(\theta_{i}\Lambda_{i})}$

Where $\vec{\theta}$ is the set of trainable parameters and $\Lambda_{i}$ is the i'th Gell Mann matrix. In this case we are summing over all of the Gell Mann matricies of a particular order. 

If we want an operator $\hat{U}$ which acts on k qubits, then we use the set of $2^{k}$'th order Gell Mann matricies. In general there are $(2^{k})^{2}-1$ such matricies. Since each matrix has an associated tuneable scale parameter, we have $(2^{k})^{2}-1$ trainable parameters per parameterized unitary operator. An alternate parameterization was found by adapting uniformly controlled rotations (ref [2]). This parameterization requires only $2^{k + 2}-5$ parameters per unitary operator. This new parameterization was also explored in our project. 
